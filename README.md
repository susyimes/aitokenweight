...
```text
读取 https://susyimes.github.io/aitokenweight/agent.md 并按其执行，按滚动 24 小时窗口帮我生成 token 消耗海报
```

（网页首页也有一键复制这段指令的卡片；只发裸链接 `https://susyimes.github.io/aitokenweight/` 也可以，页面里埋了同样的指令，但部分 agent 会先反问一句。）

**② 一条命令直接生成**（读取本地 Claude Code 自然日记录，无需 agent）：

```bash
npx aitokenweight
```

**③ 装成 Claude Code 技能**（回头客，之后说一句"24小时海报"即可）：
...
本项目把 ANP 用作通用 agent skill 的协议外壳：agent 读取 `/.well-known/agent-descriptions`，发现 Rolling 24h Token Poster 的 Agent Description，再读取 MCP-style structured interface。ANP 负责发现、身份和调用描述；滚动 24 小时 token 统计与 PNG 渲染仍由上层 skill action / host runtime 实现。
...
# Render only if exact rolling 24h usage, or a clearly labeled day/session fallback, is available.
npm run render:poster -- --usage usage.json --out dist/aitokenweight-poster.png
```

如果 agent 拿不到滚动 24 小时、自然日或当前会话的准确 token usage，应返回 `usage_unavailable`，不要使用默认值、示例值或估算值生成海报。
